export async function onRequestPost({ request, env }) {
  try {
    ...
    const user = await env.DB.prepare(`SELECT id,name,email,role,pin,title,active FROM users WHERE lower(email)=lower(?) AND pin=? AND COALESCE(active,1)=1`).bind(email, pin).first();
