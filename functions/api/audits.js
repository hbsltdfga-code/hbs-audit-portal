...
function hasSafetyCriticalIssue(b){
  ...
  return qs.some(q=>{
    ...
    // Only true failures on safety-critical questions force Level 2. Improvement/5 marks stay within normal score bands.
...
async function assignAuditTraining(env,b,id,ref,result){
  try{
    ...
    const assigned=b.audit_date||new Date().toISOString().slice(0,10);
    const engineer=b.engineer_name||''; if(!engineer) return;
    const manager=b.auditor||b.manager_name||'';
    const score=Number(b.score||0);
    const action=trainingFor(score,result,hasSafetyCriticalIssue(b));
    if(action){
...
export async function onRequestPost({request,env}){try{const b=await request.json();if(!['manager','senior_engineer'].includes(b.role||'')) return Response.json({ok:false,error:'Only managers or senior engineers can create audits'},{status:403});const score=Number(b.score||0), result=b.result||outcome(score);const cols=await tableColumns(env,'audits');const clientCol=pickClientColumn(cols);const data={engineer_name:b.engineer_name||'',site_name:b.site_name||'',audit_date:b.audit_date||new Date().toISOString().slice(0,10),auditor:b.auditor||'',appliance_type:b.audit_type||b.appliance_type||'',manufacturer:b.manufacturer||'',model:b.model||'',asset_number:b.asset_number||'',serial_number:b.serial_number||'',score,result,findings:b.findings||'',training_required:b.training_required||'',audit_json:JSON.stringify((()=>{const {photos,photo_data,photoData,...rest}=b;return rest})())};if(clientCol) data[clientCol]=b.client||b.client_name||'';const useCols=Object.keys(data).filter(c=>!cols.length||cols.includes(c));if(!useCols.length) throw new Error('No matching audit columns found.');const sql=`INSERT INTO audits (${useCols.join(',')}) VALUES (${useCols.map(()=>'?').join(',')})`;const ins=await env.DB.prepare(sql).bind(...useCols.map(c=>data[c])).run();const id=ins.meta?.last_row_id;const ref=`HBS-${id}`;await assignAuditTraining(env,b,id,ref,result);try{const photos=Array.isArray(b.photos)?b.photos:[];if(id&&photos.length){for(const p of photos.slice(0,8)){const name=p.filename||p.photo_name||'audit-photo.jpg';const url=p.data_url||p.photo_url||'';if(url&&url.length<900000){await env.DB.prepare('INSERT INTO audit_photos (audit_id, photo_name, photo_url) VALUES (?,?,?)').bind(id,name,url).run()}}}}catch(photoErr){console.log('Photo save skipped:',photoErr.message)}return Response.json({ok:true,id,ref,score,result})}catch(e){return Response.json({ok:false,error:e.message},{status:500})}}
