...
async function ensureAuditPhotos(env){
 await env.DB.prepare(`CREATE TABLE IF NOT EXISTS audit_photos (id INTEGER PRIMARY KEY AUTOINCREMENT,audit_id INTEGER NOT NULL,photo_name TEXT,photo_url TEXT,created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
}
