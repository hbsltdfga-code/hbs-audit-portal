...
function trainingText(body, score) {
  let level = body.test_level || (score < 75 ? 'level2' : score < 85 ? 'level1' : 'none');
  if (level === 'auto') level = score < 75 ? 'level2' : score < 85 ? 'level1' : 'none';
  if (level === 'level2') return 'Level 2 Advanced Competency Assessment required. Re-audit and practical review to be completed.';
  if (level === 'level1') return 'Level 1 Commercial Gas Safety Refresher required. Re-audit to be completed within 30 days.';
  return body.training_required || 'Post audit refresher training / toolbox talk';
}
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (Array.isArray(body.photos)) {
      for (const p of body.photos.slice(0,12)) {
        if (p.data_url) {
    ...
    if (score < 85) {
      await env.DB.prepare(`INSERT INTO training_records
        ...
        VALUES (?,?,?,?,?)`)
        .bind(body.engineer_name || '', trainingText(body, score), new Date().toISOString().slice(0,10), '', 'Open').run();
