...
export async function onRequestGet({request, env}) {
  try {
    ...
    if (id) {
      ...
      if (openedBy) {
        await env.DB.prepare('INSERT INTO library_access_log (document_id, opened_by) VALUES (?,?)')
          .bind(Number(id), openedBy).run();
      }

      return Response.json({
        ok:true,
        document:{...doc, file_url:`/api/library-file?id=${encodeURIComponent(id)}`}
      });
    }
      ...
      rows = await env.DB.prepare(`
        ...
        WHERE category=?
          AND (
            ...
            OR lower(uploaded_by) LIKE ?
          )
        ORDER BY datetime(created_at) DESC, id DESC
        LIMIT 1000
      `).bind(cat, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`).all();
    } else if (q) {
      rows = await env.DB.prepare(`
        SELECT * FROM technical_library
