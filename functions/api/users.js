...
export async function onRequestGet({ env }) {
  try {
    ...
    const rows = await env.DB.prepare(`
      ...
      ORDER BY
        CASE role WHEN 'manager' THEN 1 WHEN 'senior_engineer' THEN 2 ELSE 3 END,
        name
    `).all();
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (b.id) {
      await env.DB.prepare(`
        UPDATE users SET name=?, email=?, role=?, pin=?, title=?, active=? WHERE id=?
      `).bind(
        b.name || '',
        b.email || '',
        b.role || 'engineer',
    ...
    const ins = await env.DB.prepare(`
      INSERT INTO users (name,email,role,pin,title,active) VALUES (?,?,?,?,?,?)
    `).bind(
      b.name || '',
      b.email || '',
      b.role || 'engineer',
