...
export async function onRequestPost({request,env}){try{await ensure(env);const b=await request.json();const assigned=b.assigned_date||new Date().toISOString().slice(0,10);const due=b.due_date||addDays(assigned,30);const ins=await env.DB.prepare('INSERT INTO training_records (engineer_name,training_type,assigned_date,due_date,status,audit_ref,manager_name,manager_notes) VALUES (?,?,?,?,?,?,?,?)').bind(b.engineer_name||'',b.training_type||'Training Action',assigned,due,b.status||'Open',b.audit_ref||'',b.manager_name||'',b.manager_notes||'').run();return Response.json({ok:true,id:ins.meta?.last_row_id})}catch(e){return Response.json({ok:false,error:e.message},{status:500})}}
